...
def studentInput(user_grades):
    ...
    while True:
        if student_ID in user_grades:
            pword=raw_input('Please type in your password: ')
            m=md5.new()
            m.update(pword)
            while True:
                #need to decrypt m before being able to compare it to user_grades[student_ID]["password"]
                if pword==user_grades[student_ID]["password"]:
                    return student_ID
                else:
                    pword=raw_input("Your password is not correct")
